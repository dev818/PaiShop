...
class LiveVideoPlayBackVC: UIViewController {
    ...
    @objc func timerRun(_ sender:Timer){
        ...
        self.timeLeftLabel.text = getMMSSFromSS(self.aliyunVodPlayer.currentTime)
        self.timeSlider.setValue(Float(self.aliyunVodPlayer.currentTime/self.aliyunVodPlayer.duration), animated: true)
    }
...
extension LiveVideoPlayBackVC: AliyunVodPlayerDelegate {
    func vodPlayer(_ vodPlayer: AliyunVodPlayer!, onEventCallback event: AliyunVodPlayerEvent) {
        ...
        case AliyunVodPlayerEvent.prepareDone:
            ...
            self.timeRightLabel.text = self.getMMSSFromSS(aliyunVodPlayer.duration)
        case AliyunVodPlayerEvent.play:
            //暂停后恢复播放时触发
            print("AliyunVodPlayerEvent.play")
            self.isPause = false
            self.isFinished = false
            self.videoPlayButton.setImage(ImageAsset.social_music_pause.image, for: .normal)
            break;
        ...
        case AliyunVodPlayerEvent.pause:
            ...
            self.isPause = true
            self.videoPlayButton.setImage(ImageAsset.social_music_play.image, for: .normal)
            //视频暂停时触发
            break;
...
extension LiveVideoPlayBackVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == productTableView {
            if indexPath.section == 0 {
                ...
                cell.setPlayerBackCellContent(videoModel.store!, playBackVC: self)
                return cell
            }
...
extension LiveVideoPlayBackVC: UITextFieldDelegate {
...
extension LiveVideoPlayBackVC: WebSocketDelegate {
