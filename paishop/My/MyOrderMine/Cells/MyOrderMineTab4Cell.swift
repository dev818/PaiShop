...
class MyOrderMineTab4Cell: UITableViewCell {
    ...
    @IBAction func confirmReceiption(_ sender: UIButton) {
            ...
            if success {
                ...
                DispatchQueue.main.async {
                    if self.vc is MyOrderMineTab1VC {
                        ...
                        (self.vc as! MyOrderMineTab4VC).loadOrderItems(resetData: true, loadFirst: true)
                    }
                }
            } else {
                // try again...
                MyAPI.shared.orderStatusChange(params: parameters, completion: { (json1, success1) in
                    ProgressHUD.dismiss()
                    sender.isEnabled = true
                    if success1 {
                        ProgressHUD.showSuccessWithStatus("成功更改状态")
