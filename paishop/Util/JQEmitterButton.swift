...
class JQEmitterButton: UIButton {
    ...
    @objc fileprivate func buttonClick() {
        ...
        UIView.animate(withDuration: upDuration, delay: 0, options: .curveLinear, animations: {
            blingImageView.center = CGPoint(x: self.bounds.width/2+CGFloat(arc4random_uniform(80))-40, y: self.bounds.size.height/2-50)
        }) { (_) in
            UIView.animate(withDuration: upDuration, delay: 0, options: .curveLinear, animations: {
    ...
    fileprivate func bubbleAnimation(start: CGPoint, isAuto: Bool) {
        ...
        let bubble = UIImageView(image: UIImage(named: "social_praise_sm\(arc4random_uniform(6))"))
